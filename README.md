...
| `COOKIES_NAME` | `Nginx-Auth-Server-Token` | Set cookie name attribute for cookie. Useful if you are running more than one auth server on the same domain. The cookie name can contain any US-ASCII characters, but some exception apply (refer to https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie) |
